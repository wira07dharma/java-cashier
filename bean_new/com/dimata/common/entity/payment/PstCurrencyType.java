...
public class  PstCurrencyType extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc, I_Language  {
    ...
    public static Vector list(int limitStart, int recordToGet, String whereClause, String order) {
        ...
        try {
            ...
            if (whereClause != null && whereClause.length() > 0){
                ...
                sql = sql + " AND "+fieldNames[FLD_INCLUDE_IN_PROCESS]+"=1 ";
            }else{
                sql = sql + " WHERE " + fieldNames[FLD_INCLUDE_IN_PROCESS]+"=1 ";
            }    
    ...
    public static double getExchangeRate(String whereClause) {
        DBResultSet dbrs = null;
        try {
            String sql = "SELECT SELLING_RATE FROM daily_rate ";
            if (whereClause != null && whereClause.length() > 0) {
                sql = sql + " WHERE " + whereClause + " ORDER BY ROSTER_DATE DESC LIMIT 0,1";
            }
