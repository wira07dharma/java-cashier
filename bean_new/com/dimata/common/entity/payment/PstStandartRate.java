...
public class PstStandartRate extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc, I_Language, I_StandardRate {
    ...
    /** Fungsi ini digunakan untuk mendapatkan list standard rate yang aktif, dimana result disimpan dalam hashtable
     * create by: gwawan@dimata 25/11/2007 2:33 PM
     * @param
     * @return Hashtable
     */
    ...
    public static String getCurrency(long oid){
         ...
         *  SELECT DISTINCT ct.`CURRENCY` FROM pos_price_type_mapping AS pptm
            INNER JOIN standard_rate AS sr
            ON sr.`STANDARD_RATE_ID`=pptm.`STANDART_RATE_ID`
            INNER JOIN country AS ct
            ON ct.`COUNTRY_ID`=sr.`COUNTRY_ID`
            WHERE sr.`STANDARD_RATE_ID`='504404518843535527';
         */
