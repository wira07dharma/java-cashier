    ...
    public static Vector getListTransaksiKredit(int limitStart, int recordToGet, String whereClause, String order) {
        ...
        try {
            String sql = "SELECT * "
                    ...
                    + " ON a." + PstContactList.fieldNames[PstContactList.FLD_CONTACT_ID] + " = p." + PstPinjaman.fieldNames[PstPinjaman.FLD_ANGGOTA_ID]
                    + "";
            if (whereClause != null && whereClause.length() > 0) {
                sql = sql + " WHERE " + whereClause;
            }
        ...
        public static Vector getListTransaksiKreditCount(int limitStart, int recordToGet, String whereClause, String order) {
        Vector lists = new Vector();
        DBResultSet dbrs = null;
        try {
            String sql = "SELECT COUNT(" + PstTransaksi.fieldNames[PstTransaksi.FLD_TRANSAKSI_ID] + ")"
                    + " FROM " + PstTransaksi.TBL_TRANSAKSI + " AS t "
                    + "";
            if (whereClause != null && whereClause.length() > 0) {
                sql = sql + " WHERE " + whereClause;
