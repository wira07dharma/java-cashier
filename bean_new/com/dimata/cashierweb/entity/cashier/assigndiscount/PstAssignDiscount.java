 ...
 */
package com.dimata.cashierweb.entity.cashier.assigndiscount;
...
public class PstAssignDiscount extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc, I_Language {
    ...
    public static final String TBL_ASSIGNDISCOUNT = "pos_assign_discount";
    ...
    public static Vector list(int limitStart, int recordToGet, String whereClause, String order) {
        ...
        try {
            String sql = "SELECT * FROM " + TBL_ASSIGNDISCOUNT;
    ...
    public static boolean checkOID(long entAssignDiscountId) {
        ...
        try {
            String sql = "SELECT * FROM " + TBL_ASSIGNDISCOUNT + " WHERE "
    ...
    public static int getCount(String whereClause) {
        DBResultSet dbrs = null;
        try {
            String sql = "SELECT COUNT(" + PstAssignDiscount.fieldNames[PstAssignDiscount.FLD_ASSIGN_DISC_ID] + ") FROM " + TBL_ASSIGNDISCOUNT;
            if (whereClause != null && whereClause.length() > 0) {
                sql = sql + " WHERE " + whereClause;
            }
            dbrs = DBHandler.execQueryResult(sql);
    ...
    public static int getMaxDiscount(String whereClause) {
        DBResultSet dbrs = null;
        try {
            String sql = "SELECT " + PstAssignDiscount.fieldNames[PstAssignDiscount.FLD_MAX_DISC] + " FROM " + TBL_ASSIGNDISCOUNT;
            if (whereClause != null && whereClause.length() > 0) {
                sql = sql + " WHERE " + whereClause;
            }
            dbrs = DBHandler.execQueryResult(sql);
            ...
            int count = 0;
            while (rs.next()) {
                count = rs.getInt(fieldNames[FLD_MAX_DISC]);
            }
            rs.close();
            return count;
        } catch (Exception e) {
