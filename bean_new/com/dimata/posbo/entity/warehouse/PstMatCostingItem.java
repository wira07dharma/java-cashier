...
public class PstMatCostingItem extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc, I_Language {
    ...
    public static Vector list(int limitStart,int recordToGet, long oidMatDispatch, String where) {
        ...
        try {
            ...
            ", DFI." + fieldNames[FLD_SPESIAL_NOTE] +
            ...
            while(rs.next()) {
                ...
                dfItem.setSpesialNote(rs.getString(fieldNames[FLD_SPESIAL_NOTE]));
                //jika dia composite cari hpp dan totalnya
                if(rs.getInt(13)==PstMaterial.MAT_TYPE_COMPOSITE){
    ...
    public static double getQtyByCategory(long oidCostingMaterialId, long categoryOid){
        double count = 0;
        DBResultSet dbrs = null;
        try {
            String sql = "SELECT SUM(DFI." + fieldNames[FLD_QTY] +") AS "+fieldNames[FLD_QTY]+"," + 
            " SUM(DFI."+fieldNames[FLD_QTY_COMPOSITE]+") AS "+fieldNames[FLD_QTY_COMPOSITE]+"" +
            " FROM (" + TBL_MAT_COSTING_ITEM + " DFI" +
            " INNER JOIN " + PstMaterial.TBL_MATERIAL + " MAT" +
            " ON DFI." + fieldNames[FLD_MATERIAL_ID] +
            " = MAT." + PstMaterial.fieldNames[PstMaterial.FLD_MATERIAL_ID] + ")" +
            ...
            " AND MAT."+PstMaterial.fieldNames[PstMaterial.FLD_CATEGORY_ID]+"="+categoryOid+"";
            
            dbrs = DBHandler.execQueryResult(sql);
            ResultSet rs = dbrs.getResultSet();
            while (rs.next()) {
                count = rs.getDouble(fieldNames[FLD_QTY]) + rs.getDouble(fieldNames[FLD_QTY_COMPOSITE]);
            }
            rs.close();
    ...
    public static double getQty(long oidCostingMaterialId) {
        double count = 0;
        DBResultSet dbrs = null;
        try {
            String sql = "SELECT SUM("+fieldNames[FLD_QTY]+") AS "+fieldNames[FLD_QTY]+", "
                + " SUM("+fieldNames[FLD_QTY_COMPOSITE]+") AS "+fieldNames[FLD_QTY_COMPOSITE]+"" 
                + " FROM "+TBL_MAT_COSTING_ITEM+" WHERE "+fieldNames[FLD_COSTING_MATERIAL_ID]+"='"+oidCostingMaterialId+"'"
                + " AND "+fieldNames[FLD_PARENT_ID]+"=0";
