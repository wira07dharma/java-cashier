...
public class PstMatCosting extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc, I_Language, I_Document {
    ...
    //add column cashCashierId
    public static final int FLD_CASH_CASHIER_ID = 13;
    
    //add colum enableStockFisik
    ...
    public static long insertExc(MatCosting matCosting) throws DBException {
        try {
            ...
            pstMatCosting.setLong(FLD_RESERVATION_ID,matCosting.getReservationId());
    ...
    public static Vector listJoinContactLocation(int limitStart, int recordToGet, String whereClause, String order){
         Vector lists = new Vector();
        DBResultSet dbrs = null;
        try {
            String sql = ""
                + " SELECT pcm."+PstMatCosting.fieldNames[PstMatCosting.FLD_COSTING_MATERIAL_ID]+","
                + " pcm."+PstMatCosting.fieldNames[PstMatCosting.FLD_LOCATION_ID]+","
                + " pcm."+PstMatCosting.fieldNames[PstMatCosting.FLD_COSTING_CODE]+","
                + " pcm."+PstMatCosting.fieldNames[PstMatCosting.FLD_COSTING_DATE]+","
                + " pcm."+PstMatCosting.fieldNames[PstMatCosting.FLD_REMARK]+","              
                + " pcm."+PstMatCosting.fieldNames[PstMatCosting.FLD_CASH_CASHIER_ID]+","
                + " pcm."+PstMatCosting.fieldNames[PstMatCosting.FLD_CONTACT_ID]+","
                + " loc."+PstLocation.fieldNames[PstLocation.FLD_NAME]+","                
                + " cl."+PstContactList.fieldNames[PstContactList.FLD_PERSON_NAME]+""
                + " FROM"
                + " "+PstMatCosting.TBL_COSTING+" pcm"
                + " LEFT JOIN "+PstLocation.TBL_P2_LOCATION+" loc"
                + " ON pcm."+PstMatCosting.fieldNames[PstMatCosting.FLD_LOCATION_ID]+" = loc."+PstLocation.fieldNames[PstLocation.FLD_LOCATION_ID]+""
                + " LEFT JOIN "+PstContactList.TBL_CONTACT_LIST+" cl"
                + " ON pcm."+PstMatCosting.fieldNames[PstMatCosting.FLD_CONTACT_ID]+" = cl."+PstContactList.fieldNames[PstContactList.FLD_CONTACT_ID]+"";
            ...
            while (rs.next()) {
                MatCosting matCosting = new MatCosting();
                resultToObjectJoinContactLocation(rs, matCosting);
                lists.add(matCosting);
            }
            rs.close();
            return lists;
    ...
    public static void resultToObjectJoinContactLocation(ResultSet rs, MatCosting matCosting) {
        try {
            matCosting.setOID(rs.getLong(PstMatCosting.fieldNames[PstMatCosting.FLD_COSTING_MATERIAL_ID]));
            matCosting.setLocationId(rs.getLong(PstMatCosting.fieldNames[PstMatCosting.FLD_LOCATION_ID]));           
            matCosting.setCostingDate(rs.getDate(PstMatCosting.fieldNames[PstMatCosting.FLD_COSTING_DATE])); 
            ...
            matCosting.setCashCashierId(rs.getLong(PstMatCosting.fieldNames[PstMatCosting.FLD_CASH_CASHIER_ID]));                       
            matCosting.setContactId(rs.getLong(PstMatCosting.fieldNames[PstMatCosting.FLD_CONTACT_ID]));
            matCosting.setLocationName(rs.getString(PstLocation.fieldNames[PstLocation.FLD_NAME]));
            matCosting.setPersonName(rs.getString(PstContactList.fieldNames[PstContactList.FLD_PERSON_NAME]));
        } catch (Exception e) {
        }
    }    
