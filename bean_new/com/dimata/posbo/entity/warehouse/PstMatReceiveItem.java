...
public class PstMatReceiveItem extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc, I_Language {
	...
	public static String getInsertSQL(MatReceiveItem matreceiveitem){
		String sql = "";
		try {
			PstMatReceiveItem pstMatReceiveItem = new PstMatReceiveItem(matreceiveitem.getOID());
			sql = pstMatReceiveItem.getInsertSQL();
		} catch (Exception exc){
     ...
     */
      public static Vector listLastReceive(long oidMaterial,int limitStart, int recordToGet, String whereClause, String order) {
        ...
        try {
            ...
            String sql = " SELECT  PRM."+PstMatReceive.fieldNames[PstMatReceive.FLD_RECEIVE_MATERIAL_ID]+", PRM."+PstMatReceive.fieldNames[PstMatReceive.FLD_REC_CODE]+", PRM."+PstMatReceive.fieldNames[PstMatReceive.FLD_RECEIVE_DATE]+", PRMI.MATERIAL_ID, PRMI.COST, PRM.SUPPLIER_ID, PRM."+PstMatReceive.fieldNames[PstMatReceive.FLD_TRANS_RATE]+" FROM " + PstMatReceive.TBL_MAT_RECEIVE + " AS PRM "+
