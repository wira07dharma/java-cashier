...
public class SessMatCostingStokFisik {
	...
	public static double qtyMaterialBasedOnStockCardNoSale(long materialOid, long locationOid, Date dtstartReposting, int status) {
		...
		double qtyReal = 0.0;
		try {
			Formater.formatDate(dtstartReposting, "dd-MM-yyyy", "dd-mm-yyyy"); // (untuk apa? ianrizky 2018-06-08)
			...
			if (dtLastOpnameDate != null) {
				...
				dtstart = PstPeriode.getPeriodeRunning().getStartDate();
			}
	...
	// Created by ianrizky at 2018-06-13
	public static double qtyMaterialBasedOnOpeningCashier(long materialOid, Date startOpening) {

		double realTimeStock = 0;
		DBResultSet dbrs = null;
		try {
			String sql = ""
					+ "SELECT"
						+ " SUM(" + PstBillDetail.TBL_CASH_BILL_DETAIL + "." + PstBillDetail.fieldNames[PstBillDetail.FLD_QUANTITY] + ") AS REALTIME_STOCK"
					+ " FROM "
						+ PstBillDetail.TBL_CASH_BILL_DETAIL
						+ " INNER JOIN " + PstBillMain.TBL_CASH_BILL_MAIN
							+ " ON " + PstBillDetail.TBL_CASH_BILL_DETAIL + "." + PstBillDetail.fieldNames[PstBillDetail.FLD_BILL_MAIN_ID] + " = " + PstBillMain.TBL_CASH_BILL_MAIN + "." + PstBillMain.fieldNames[PstBillMain.FLD_BILL_MAIN_ID]
						+ " INNER JOIN " + PstCashCashier.TBL_CASH_CASHIER
							+ " ON " + PstBillMain.TBL_CASH_BILL_MAIN + "." + PstBillMain.fieldNames[PstBillMain.FLD_CASH_CASHIER_ID] + " = " + PstCashCashier.TBL_CASH_CASHIER + "." + PstCashCashier.fieldNames[PstCashCashier.FLD_CASH_CASHIER_ID]
					+ " WHERE " + PstBillDetail.TBL_CASH_BILL_DETAIL + "." + PstBillDetail.fieldNames[PstBillDetail.FLD_MATERIAL_ID] + " = " + materialOid + " AND " + PstCashCashier.TBL_CASH_CASHIER + "." + PstCashCashier.fieldNames[PstCashCashier.FLD_SPVCLOSE_OID] + " = 1 "
					+ " AND " + PstBillMain.TBL_CASH_BILL_MAIN + "." + PstBillMain.fieldNames[PstBillMain.FLD_TRANSACTION_STATUS] + " != 2";

			dbrs = DBHandler.execQueryResult(sql);
			ResultSet rs = dbrs.getResultSet();
			while (rs.next()) {
				realTimeStock = rs.getDouble("REALTIME_STOCK");
			}
			rs.close();
		} catch (Exception ex) {
			System.out.println("err getLastOpnameDate : " + ex.toString());
		} finally {
	...
	public static double qtyMaterialBasedOnOpeningCashierInvoice(long materialOid, Date startOpening) {
		...
		try {
			String sql = ""
					...
					+ " WHERE " + PstBillDetail.TBL_CASH_BILL_DETAIL + "." + PstBillDetail.fieldNames[PstBillDetail.FLD_MATERIAL_ID] + " = " + materialOid + " AND " + PstCashCashier.TBL_CASH_CASHIER + "." + PstCashCashier.fieldNames[PstCashCashier.FLD_SPVCLOSE_OID] + " = 1 "
					+ " AND " + PstBillMain.TBL_CASH_BILL_MAIN + "." + PstBillMain.fieldNames[PstBillMain.FLD_TRANSACTION_STATUS] + " != 2"
					+ " AND " + PstBillMain.TBL_CASH_BILL_MAIN + "." + PstBillMain.fieldNames[PstBillMain.FLD_DOC_TYPE] + " = "+PstBillMain.TYPE_INVOICE;
	...
	public static double qtyMaterialBasedOnOpeningCashierReturn(long materialOid, Date startOpening) {
		...
		try {
			String sql = ""
					...
					+ " WHERE " + PstBillDetail.TBL_CASH_BILL_DETAIL + "." + PstBillDetail.fieldNames[PstBillDetail.FLD_MATERIAL_ID] + " = " + materialOid + " AND " + PstCashCashier.TBL_CASH_CASHIER + "." + PstCashCashier.fieldNames[PstCashCashier.FLD_SPVCLOSE_OID] + " = 1 "
					+ " AND " + PstBillMain.TBL_CASH_BILL_MAIN + "." + PstBillMain.fieldNames[PstBillMain.FLD_TRANSACTION_STATUS] + " != 2"
					+ " AND " + PstBillMain.TBL_CASH_BILL_MAIN + "." + PstBillMain.fieldNames[PstBillMain.FLD_DOC_TYPE] + " = "+PstBillMain.TYPE_RETUR;
	...
	public static Vector getLastDateOpnameReposting(long oidLocation, long oidMaterial, Date dtstart) {
		...
		try {
			String sql = " SELECT OPN." + PstMatStockOpname.fieldNames[PstMatStockOpname.FLD_STOCK_OPNAME_DATE]
					...
					+ " FROM " + PstMatStockOpname.TBL_MAT_STOCK_OPNAME + " OPN "
