...
public class PurchaseOrderNoShippingPrintPDF extends HttpServlet {
            ...
            if (useBarcodeorSku.equals("Not initialized")) {
             ...
             chooseTypeViewSkuOrBcd = Integer.parseInt(useBarcodeorSku);
                ...
                if (oidPo != 0) {
                    ...
                    document.add(PurchaseOrderNoShippingPrintPDF.getHeaderImage(SESS_LANGUAGE, gambar,typeRequest));
                    document.add(PurchaseOrderNoShippingPrintPDF.getHeader(SESS_LANGUAGE, purchaseOrder,typeRequest));
    ...
    private static Table getHeaderImage(int SESS_LANGUAGE, com.lowagie.text.Image gambar, int typeRequest) throws BadElementException, DocumentException {
    ...
    private static Table getHeader(int SESS_LANGUAGE, PurchaseOrder purchaseOrder, int typeRequest) throws BadElementException, DocumentException {
        ...
        if (purchaseOrder.getOID() != 0) {
            ...
            table.addCell(new Phrase(contactList.getCompName(), PurchaseOrderNoShippingPrintPDF.fontHeader));
    ...
    private static Document getContent(Vector vct, Document document, PdfWriter writer, int SESS_LANGUAGE, PurchaseOrder purchaseOrder) throws BadElementException, DocumentException {
        ...
        if (vct != null && vct.size() > 0) {
            try {
                ...
                /** end of table **/
                document.add(table);
                document.add(PurchaseOrderNoShippingPrintPDF.getHeaderLast(SESS_LANGUAGE, purchaseOrder, totalQty, total));
            } catch (Exception e) {
        ...
        if (vct != null && vct.size() > 0) {
            try {
                ...
                */
                ...
                document.add(table);
                document.add(PurchaseOrderNoShippingPrintPDF.getHeaderLastNoValue(SESS_LANGUAGE, purchaseOrder, totalQty, total));
            } catch (Exception e) {
                System.out.println("exc contenct" + e.toString());
            }
        }
    ...
    private static Table getHeaderLastNoValue(int SESS_LANGUAGE, PurchaseOrder purchaseOrder, double totalQty, double total) throws BadElementException, DocumentException {
        int ctnInt[] = {10, 1, 60, 10, 1, 15};
        //Table table = new Table(10);
        Table table = new Table(6);
        try {
