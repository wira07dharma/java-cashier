...
public class PstCashReturn  extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc, I_Language {
	...
	public static String getInsertSQL(CashReturn cashReturn){
		String sql = "";
		try {
			PstCashReturn pstCashReturn = new PstCashReturn(cashReturn.getOID());
			sql = pstCashReturn.getInsertSQL();
		} catch (Exception exc){
    ...
    public static double getReturnSummary2(long oidCashCashier, long currencyId) {
      double count = 0;
        DBResultSet dbrs = null;
        try {
            String sql = "SELECT SUM(CRP." + fieldNames[FLD_AMOUNT] + "* CRP." + fieldNames[FLD_RATE] + ")" +
            " FROM " + TBL_RETURN + " CRP" +
            " INNER JOIN " + PstBillMain.TBL_CASH_BILL_MAIN + " CBM" +
            " ON CRP." + fieldNames[FLD_BILLMAIN_ID] +
            " = CBM." + PstBillMain.fieldNames[PstBillMain.FLD_BILL_MAIN_ID] +
            ...
            + " WHERE CBM."+PstBillMain.fieldNames[PstBillMain.FLD_DOC_TYPE]+"=1)" +
            //" AND CBM." + PstBillMain.fieldNames[PstBillMain.FLD_TRANSACTION_STATUS] + "!=2" +
            " AND CRP."+fieldNames[FLD_CURRENCY_ID]+" ='"+currencyId+"'" +
            " AND CBM." + PstBillMain.fieldNames[PstBillMain.FLD_TRANSACTION_STATUS] + "=0" +
            " AND CBM." + PstCashCashier.fieldNames[PstCashCashier.FLD_CASH_CASHIER_ID]+
            " = " + oidCashCashier;
