...
public class  PstCashCashier extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc, I_Language{
	...
	// close note
	public static final int FLD_CLOSE_NOTE = 10;
    ...
    public static CashCashier fetchExc(long oid) throws DBException {
        try {
			...
			cashCashier.setCloseNote(pstCashCashier.getString(FLD_CLOSE_NOTE));
    ...
    public static long insertExc(CashCashier cashCashier) throws DBException {
        try {
			...
			pstCashCashier.setString(FLD_CLOSE_NOTE, cashCashier.getCloseNote());
	...
	public static String getInsertSQL(CashCashier cashCashier){
		String sql = "";
		try {
			PstCashCashier pstCashCashier = new PstCashCashier(cashCashier.getOID());
			sql = pstCashCashier.getInsertSQL();
		} catch (Exception exc){
    ...
    public static Vector listDistinct(int limitStart,int recordToGet, String whereClause, String order){
        ...
        try {
            String sql = ""
                + " SELECT"
                + " DISTINCT(DATE(cc."+PstCashCashier.fieldNames[PstCashCashier.FLD_OPEN_DATE]+"))"
                + " AS "+PstCashCashier.fieldNames[PstCashCashier.FLD_OPEN_DATE]+","
                + " cm."+PstCashMaster.fieldNames[PstCashMaster.FLD_LOCATION_ID]+"" 
                + " FROM" 
                + " "+PstCashCashier.TBL_CASH_CASHIER+" cc"
                + " INNER JOIN cash_master cm"
            ...
            while(rs.next()) {
                CashCashier cashCashier = new CashCashier();
                cashCashier.setOpenDate(rs.getDate(PstCashCashier.fieldNames[PstCashCashier.FLD_OPEN_DATE]));
                cashCashier.setLocationId(rs.getLong(PstCashMaster.fieldNames[PstCashMaster.FLD_LOCATION_ID]));
    ...
    public static Vector listJoin(int limitStart, int recordToGet, String whereClause, String order) {
        ...
        try {
            String sql = ""
                + " SELECT CSH.* , MSTR."+PstCashMaster.fieldNames[PstCashMaster.FLD_CASH_MASTER_ID]+" , "
                ...
                + " INNER JOIN "
                + " "+PstCashMaster.TBL_CASH_MASTER+" MSTR "
    ...
    public static void resultToObject(ResultSet rs, CashCashier cashCashier) {
        try {
			...
			cashCashier.setCloseNote(rs.getString(PstCashCashier.fieldNames[PstCashCashier.FLD_CLOSE_NOTE]));
    ...
    public static void resultToObjectOpening(ResultSet rs, OpeningCashCashier openingCashCashier) {
        try {
            ...
            openingCashCashier.setAppUserId(rs.getLong(PstCashCashier.fieldNames[PstCashCashier.FLD_APPUSER_ID]));
	...
	public static int getDayOfSale(String whereClause) {
        int count = 0;
        DBResultSet dbrs = null;
        try {
            String sql = "SELECT COUNT(DISTINCT DATE_FORMAT("+PstCashCashier.fieldNames[PstCashCashier.FLD_OPEN_DATE]+",'%Y-%m-%d')) "
