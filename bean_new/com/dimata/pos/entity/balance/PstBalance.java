...
public class PstBalance extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc, I_Language {
  ...
  //constructor
	...
	public static String getInsertSQL(Balance balance){
		String sql = "";
		try {
			PstBalance pstBalance = new PstBalance(balance.getOID());
			sql = pstBalance.getInsertSQL();
		} catch (Exception exc){
    ...
    public static Vector listJoinCashCashiermaster(int limitStart,int recordToGet, String whereClause, String order){
        Vector lists = new Vector(); 
        DBResultSet dbrs = null;
        try {
            String sql = ""
                ...
                + " AS "+PstCashCashier.fieldNames[PstCashCashier.FLD_OPEN_DATE]+"," 
                ...
                + " cm."+PstCashMaster.fieldNames[PstCashMaster.FLD_LOCATION_ID]+"," 
                + " cm."+PstCashMaster.fieldNames[PstCashMaster.FLD_CASH_MASTER_ID]+"," 
                + " cc."+PstCashCashier.fieldNames[PstCashCashier.FLD_CLOSE_DATE]+" "
                + " AS "+PstCashCashier.fieldNames[PstCashCashier.FLD_CLOSE_DATE]+"," 
                + " lc."+PstLocation.fieldNames[PstLocation.FLD_NAME]+"," 
                + " pu."+PstAppUser.fieldNames[PstAppUser.FLD_LOGIN_ID]+"," 
                + " pu."+PstAppUser.fieldNames[PstAppUser.FLD_FULL_NAME]+"" 
                + " FROM" 
                + " "+PstBalance.TBL_BALANCE+" cb"
                + " INNER JOIN "+PstCashCashier.TBL_CASH_CASHIER+" cc "
                + " ON cb."+PstBalance.fieldNames[PstBalance.FLD_CASH_CASHIER_ID]+" = cc."+PstCashCashier.fieldNames[PstCashCashier.FLD_CASH_CASHIER_ID]+""
                + " INNER JOIN "+PstCashMaster.TBL_CASH_MASTER+" cm "
                + " ON cc."+PstCashCashier.fieldNames[PstCashCashier.FLD_CASHMASTER_ID]+" = cm."+PstCashMaster.fieldNames[PstCashMaster.FLD_CASH_MASTER_ID]+""
                + " INNER JOIN "+PstLocation.TBL_P2_LOCATION+" lc "
                + " ON cm."+PstCashMaster.fieldNames[PstCashMaster.FLD_LOCATION_ID]+" = lc."+PstLocation.fieldNames[PstLocation.FLD_LOCATION_ID]+"" 
                + " INNER JOIN "+PstAppUser.TBL_APP_USER+" pu "
                + " ON cc."+PstCashCashier.fieldNames[PstCashCashier.FLD_APPUSER_ID]+" = pu."+PstAppUser.fieldNames[PstAppUser.FLD_USER_ID]+"";
            ...
            while(rs.next()) {
                ...
                cashCashier.setOpenDate(rs.getTimestamp(PstCashCashier.fieldNames[PstCashCashier.FLD_OPEN_DATE]));
                ...
                cashCashier.setCloseDate(rs.getTimestamp(PstCashCashier.fieldNames[PstCashCashier.FLD_CLOSE_DATE]));       
                listTemp.add(cashCashier);
