...
public class PstBillMain extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc, I_Language {
    ...
    /**
     * Untuk Total Penjualan AriWiweka 20130711
     */
    ...
    /**
     * Added by Ari Ardiadi, September, 16th 2015 for setting taxInclude
     */
    public static final int FLD_TAX_INCLUDE = 52;
    // add opie -eyek 2015-11-27 untuk penambhan reservasi
    public static final int FLD_RESERVATION_ID = 53;

    public static final int FLD_TABLE_NAME = 54;
    public static final int FLD_ID_NEGARA = 55;
    public static final int FLD_GENDER = 56;

    //ADDED BY DEWOK 20191002 FOR DUTY FREE
    public static final int FLD_SHIPPING_DATE = 57;
    public static final int FLD_FLIGHT_NUMBER = 58;
    public static final int FLD_STATUS = 59;
    ...
    /**
     * Ari Wiweka 20130716 untuk nota type salesorder dan kasir
     */
    ...
    public static BillMain fetchExc(long oid) throws DBException {
        try {
            ...
            //add opie-eyek 20151127 untuk 
            billMain.setReservasiId(pstBillMain.getlong(FLD_RESERVATION_ID));
            billMain.setTableName(pstBillMain.getString(FLD_TABLE_NAME));
    ...
    public synchronized static long insertExc(BillMain billMain) throws DBException {
        try {
            ...
            pstBillMain.setLong(FLD_RESERVATION_ID, billMain.getReservasiId());
            pstBillMain.setString(FLD_TABLE_NAME, billMain.getTableName());
    ...
    public static String getInsertSQL(BillMain billMain) {
        String sql = "";
        try {
            PstBillMain pstBillMain = new PstBillMain(billMain.getOID());
            sql = pstBillMain.getInsertSQL();
        } catch (Exception exc) {
    ...
    /**
     * Untuk Update Total Penjualan Ari_wiweka 20130701
     *
     * @param oid
     * @return
     * @throws DBException
    ...
    /**
     * Untuk Update Grand Total, Diskon, Tax Ari_wiweka 20130723
     *
     * @param oid
     * @return
     * @throws DBException
    ...
    public static int getCountTableOpenBill(int limitStart, int recordToGet, String whereClause, String order) {
        int data = 0;
        DBResultSet dbrs = null;
        try {

            String sql = " "
                    + "SELECT COUNT(DISTINCT(cbm." + PstBillMain.fieldNames[PstBillMain.FLD_TABLE_ID] + ")) AS " + fieldNames[FLD_TABLE_ID] + " "
                    + " FROM " + PstBillMain.TBL_CASH_BILL_MAIN + " cbm  "
                    + " INNER JOIN " + PstTableRoom.TBL_P2_TABLE + " pt"
                    + " ON cbm." + PstBillMain.fieldNames[PstBillMain.FLD_TABLE_ID] + " = pt." + PstTableRoom.fieldNames[PstTableRoom.FLD_TABLE_ID] + ""
                    + " LEFT JOIN " + PstContact.TBL_CONTACT + " cl"
                    + " ON cbm." + PstBillMain.fieldNames[PstBillMain.FLD_CUSTOMER_ID] + " = cl." + PstContact.fieldNames[PstContact.FLD_CONTACT_ID] + ""
                    + " LEFT JOIN " + PstRoom.TBL_P2_ROOM + " rm"
                    + " ON cbm." + PstBillMain.fieldNames[PstBillMain.FLD_ROOM_ID] + " = rm." + PstRoom.fieldNames[PstRoom.FLD_ROOM_ID] + "";
            ...
            while (rs.next()) {
                data = rs.getInt(fieldNames[FLD_TABLE_ID]);
            }
            rs.close();
            return data;
    ...
    /**
     * Untuk Update Transaction Status Ari_wiweka 20130701
     *
     * @param oid
     * @return
     * @throws DBException
    ...
    /**
     * Ari_wiweka 20130710 untuk sum kredit
     */
    ...
    /**
     * Ari_wiweka 20130710 untuk sum Return Kredit
     */
    ...
    /**
     * Untuk Update Invoice Number dan Invoice Counter Ari_wiweka 20130701
     *
     * @param oid
     * @return
     * @throws DBException
    ...
    /**
     * Untuk Update CashashierId Ari_wiweka 20130701
     *
     * @param oid
     * @return
     * @throws DBException
    ...
    /**
     * List Search Open Bill Ari wiweka 20130713
     *
     * @param limitStart
     * @param recordToGet
     * @param whereClause
     ...
     * @param whereClause
     * @param order
     * @return created : opie-eyek 20131201
     */
    ...
    /**
     * Ari wiweka 20130717 count retur untuk sales order
     *
     * @param whereClause
     * @return
     */
    ...
    public static void resultToObject(ResultSet rs, BillMain billMain) {
        try {
            ...
            billMain.setReservasiId(rs.getLong(PstBillMain.fieldNames[PstBillMain.FLD_RESERVATION_ID]));
            billMain.setTableName(rs.getString(PstBillMain.fieldNames[PstBillMain.FLD_TABLE_NAME]));
    ...
    /**
     * bobo pembuatan number invoice yang di counter berdasarkan lokasi dan
     * cashier number retrun = TTBBCCCCC TT = tahun BB = bulan CCCCC = counter
     */
    ...
    /**
     * bobo pencarian counter yang terbesar sesuai dengan lokasi dan cashier
     * number return adalah int counter yang sudah di tambah dengan 1
     */
    ...
    /**
     * update opie-eyek 20130917 update sales order apakah sudah semua ter
     * delivery dari gudang atau tidak
     *
     ...
     * @param matId
     * @param matPeriode
     * @return sql = SELECT SUM(CBD.QTY) FROM cash_bill_detail AS CBD INNER JOIN
     * cash_bill_main AS CBM ON CBM.CASH_BILL_MAIN_ID=CBD.CASH_BILL_MAIN_ID
     * WHERE CBM.DOC_TYPE=0 AND CBM.TRANS_TYPE=0 AND CBM.TRANSACTION_STATUS=1
     * AND CBD.MATERIAL_ID='504404535339339794' AND (CBM.STATUS_INVOICING=0 OR
     * CBM.STATUS_INVOICING=1);
     */
    ...
    public static double getStockBookingNonSalesOrder(long locId, long matId) {
        ...
        try {
            /**
             * SELECT (CBD.QTY), CBM.cash_bill_main_id,
             * CBM.PARENT_ID,CBM.CASH_CASHIER_ID FROM cash_bill_detail AS CBD
             * INNER JOIN cash_bill_main AS CBM ON
             * CBM.CASH_BILL_MAIN_ID=CBD.CASH_BILL_MAIN_ID WHERE
             * CBD.MATERIAL_ID='504404384564247000' AND parent_sales_order_id=0
             * AND CASH_CASHIER_ID!=1;
             */
            String sql = " SELECT SUM(CBD." + PstBillDetail.fieldNames[PstBillDetail.FLD_QUANTITY] + ") FROM " + PstBillDetail.TBL_CASH_BILL_DETAIL + " AS CBD "
                    + " INNER JOIN " + TBL_CASH_BILL_MAIN + " AS CBM "
                    + " ON CBM." + PstBillMain.fieldNames[PstBillMain.FLD_BILL_MAIN_ID] + "=CBD." + PstBillDetail.fieldNames[PstBillDetail.FLD_BILL_MAIN_ID] + ""
     ...
     * @param matId
     * @return SELECT SUM(CBD.QTY) FROM cash_bill_detail AS CBD INNER JOIN
     * cash_bill_main AS CBM ON CBM.CASH_BILL_MAIN_ID=CBD.CASH_BILL_MAIN_ID
     * WHERE CBD.MATERIAL_ID='504404535339311388' AND
     * CBM.PARENT_ID='504404543014460678';
     */
    ...
    public static Vector listOpenBill(int limitStart, int recordToGet, String whereClause, String order) {
        ...
        DBResultSet dbrs = null;
        /**
         * SELECT * FROM cash_bill_main AS bm WHERE bm.DOC_TYPE=0 AND
         * bm.TRANSACTION_TYPE=0 AND bm.TRANSACTION_STATUS=1 AND
    ...
    public static double getTotalOpenBill(long appUserId) {
        double count = 0;
        double sum = 0;
        DBResultSet dbrs = null;
        try {
            String sql1 = ""
                    + " SELECT COUNT(cbm." + fieldNames[FLD_BILL_MAIN_ID] + ") FROM " + TBL_CASH_BILL_MAIN + " AS cbm"
                    ...
                    + " WHERE cbm." + fieldNames[FLD_DOC_TYPE] + "=0 "
                    + " AND cbm." + fieldNames[FLD_TRANSCATION_TYPE] + "=0 "
                    + " AND cbm." + fieldNames[FLD_TRANSACTION_STATUS] + " =1 "
                    + " AND cbm." + fieldNames[FLD_STATUS_INVOICING] + " =1 "
                    + " AND cc." + PstCashCashier.fieldNames[PstCashCashier.FLD_APPUSER_ID] + "=" + appUserId + "";
    ...
    public static long updateEventName(long oidBillMain, String eventName) {
        if (oidBillMain != 0) {
            String sql = "UPDATE " + TBL_CASH_BILL_MAIN
    ...
    public static String getQueryHitungPoin(long billMainId) {
        String sql = "SELECT "
                + " pc." + PstCategory.fieldNames[PstCategory.FLD_NAME] + " AS CATEGORY "
                + ", SUM("
                + "     cbd." + PstBillDetail.fieldNames[PstBillDetail.FLD_TOTAL_PRICE]
                + "     + cbd." + PstBillDetail.fieldNames[PstBillDetail.FLD_TOTAL_SERVICE]
                + "     + cbd." + PstBillDetail.fieldNames[PstBillDetail.FLD_TOTAL_TAX]
                + " ) AS TOTAL_PRICE "
                + ", pc." + PstCategory.fieldNames[PstCategory.FLD_POINT_PRICE] + " AS POIN_PRICE "
                + ", SUM("
                + "     ("
                + "     cbd." + PstBillDetail.fieldNames[PstBillDetail.FLD_TOTAL_PRICE]
                + "     + cbd." + PstBillDetail.fieldNames[PstBillDetail.FLD_TOTAL_SERVICE]
                + "     + cbd." + PstBillDetail.fieldNames[PstBillDetail.FLD_TOTAL_TAX]
                + "     ) / pc." + PstCategory.fieldNames[PstCategory.FLD_POINT_PRICE]
                + " ) AS POIN "
                + ", cbm." + PstBillMain.fieldNames[PstBillMain.FLD_DISCOUNT] + " AS DISC_GLOBAL "
                + ", cbm." + PstBillMain.fieldNames[PstBillMain.FLD_DISC_PCT] + " AS DISC_GLOBAL_PCT "
                + " FROM " + PstBillMain.TBL_CASH_BILL_MAIN + " AS cbm "
                + " JOIN " + PstBillDetail.TBL_CASH_BILL_DETAIL + " AS cbd "
                + " ON cbd." + PstBillDetail.fieldNames[PstBillDetail.FLD_BILL_MAIN_ID] + " = cbm." + PstBillMain.fieldNames[PstBillMain.FLD_BILL_MAIN_ID]
                + " JOIN " + PstMaterial.TBL_MATERIAL + " AS pm "
                + " ON pm." + PstMaterial.fieldNames[PstMaterial.FLD_MATERIAL_ID] + " = cbd." + PstBillDetail.fieldNames[PstBillDetail.FLD_MATERIAL_ID]
                + " JOIN " + PstCategory.TBL_CATEGORY + " AS pc "
                + " ON pc." + PstCategory.fieldNames[PstCategory.FLD_CATEGORY_ID] + " = pm." + PstMaterial.fieldNames[PstMaterial.FLD_CATEGORY_ID]
                + " WHERE cbm." + PstBillMain.fieldNames[PstBillMain.FLD_BILL_MAIN_ID] + " = '" + billMainId + "'"
                + " GROUP BY pc." + PstCategory.fieldNames[PstCategory.FLD_CATEGORY_ID]
                + "";
        return sql;
    }

    public static int getPoin(long billMainId) {
        DBResultSet dbrs = null;
        int poin = 0;
        try {
            double totalBill = getSummaryTransaction("CBM." + PstBillMain.fieldNames[PstBillMain.FLD_BILL_MAIN_ID] + " = " + billMainId);
            String sql = getQueryHitungPoin(billMainId);
            dbrs = DBHandler.execQueryResult(sql);
            ResultSet rs = dbrs.getResultSet();
            while (rs.next()) {
                if (rs.getDouble("DISC_GLOBAL") == 0) {
                    poin += rs.getInt("POIN");
                } else {
                    double newTotalPrice = rs.getDouble("TOTAL_PRICE") - (rs.getDouble("TOTAL_PRICE") / totalBill * rs.getDouble("DISC_GLOBAL"));
                    poin += Math.floor(newTotalPrice / rs.getDouble("POIN_PRICE"));
                }
            }
            rs.close();
    ...
    public static int getReturnPoin(long billMainId, int qtyReturn) {
        DBResultSet dbrs = null;
        int poin = 0;
        try {
            double totalBill = getSummaryTransaction("CBM." + PstBillMain.fieldNames[PstBillMain.FLD_BILL_MAIN_ID] + " = " + billMainId);
    ...
    public static Vector getDetailPoin(long billMainId) {
        DBResultSet dbrs = null;
        Vector listPoin = new Vector();
        try {
            String sql = getQueryHitungPoin(billMainId);
            dbrs = DBHandler.execQueryResult(sql);
            ResultSet rs = dbrs.getResultSet();
            while (rs.next()) {
                ...
                c.setPointPrice(rs.getDouble("POIN_PRICE"));
                data.add(c);
    ...
    public static String getHtmlDetailPoin(long billMainId) {
        DBResultSet dbrs = null;
        String html = "";
        html += "<div style='font-size:11px'>Poin Reward :</div>"
                + "<table class='table' style='width:25%; font-size:11px'>"
                + " <tr>"
                + "     <td>Category</td>"
                + "     <td>Total Price</td>"
                + "     <td>Poin Price</td>"
        ...
        try {
            ...
            while (rs.next()) {
                count += 1;
                totalPoin += rs.getInt("POIN");
                html += ""
                        + "<tr>"
                        + " <td>" + rs.getString("CATEGORY") + "</td>"
                        + " <td>" + Formater.formatNumber(rs.getDouble("TOTAL_PRICE"), "#,###.##") + "</td>"
                        + " <td>" + Formater.formatNumber(rs.getDouble("POIN_PRICE"), "#,###.##") + "</td>"
                        + " <td>" + rs.getInt("POIN") + "</td>"
                        + "</tr>";
            }
            html += ""
                    + "<tr>"
