...
public class PstCashMaster extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc, I_Language {
    ...
    public static long getLocation(String whereClause){
        long location = 0;
        DBResultSet dbrs = null;
        try {
            String sql = "SELECT * FROM " + TBL_CASH_MASTER+" AS cm "
                    + "INNER JOIN "+PstCashCashier.TBL_CASH_CASHIER+" AS cc "
                    + "ON cm."+fieldNames[FLD_CASH_MASTER_ID]+"=cc."+PstCashCashier.fieldNames[PstCashCashier.FLD_CASHMASTER_ID];
            if(whereClause != null && whereClause.length() > 0)
                sql = sql + " WHERE " + whereClause;
            ...
            ResultSet rs = dbrs.getResultSet();
            while(rs.next()) {
                location = rs.getLong(fieldNames[FLD_LOCATION_ID]);
            }
            rs.close();
            return location;
