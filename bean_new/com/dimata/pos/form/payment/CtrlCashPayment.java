...
public class   CtrlCashPayment extends Control implements I_Language    {
    ...
    public int actionObject(int cmd, CashPayments1 cashPayments) {
        msgString = "";
        int excCode = I_DBExceptionInfo.NO_EXCEPTION;
        int rsCode = RSLT_OK;
        int counter = 0;
        ...
        switch (cmd) {
            ...
            case Command.SAVE:
                ...
                if (cashPayment.getOID() == 0) {
                    try {
                        long oid = pstCashPayment.insertExc(cashPayments);  
                        setOid(oid);
                    } catch (DBException dbexc) {
                        excCode = dbexc.getErrorCode();
                        msgString = getSystemMessage(excCode);
                    ...
                    try {
                        long oid = pstCashPayment.updateExc(cashPayments);
                    } catch (DBException dbexc) {
                        excCode = dbexc.getErrorCode();
                        msgString = getSystemMessage(excCode);
                    } catch (Exception exc) {
