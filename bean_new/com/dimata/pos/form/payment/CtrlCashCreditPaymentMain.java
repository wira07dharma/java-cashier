...
public class CtrlCashCreditPaymentMain extends Control implements I_Language {
	...
	public int action(int cmd, long getCreditPaymentMain, long oidBillMain, double amount) {
		return action(cmd, getCreditPaymentMain, oidBillMain, amount, 0);
	}

    public int action(int cmd, long getCreditPaymentMain, long oidBillMain, double amount, long customerId) {
        msgString = "";
        ...
        switch (cmd) {
            ...
            case Command.SAVE:
				...
				creditPaymentMain.setLocationId(billMain.getLocationId());
                    ...
                    try {
                        ...
                        if (oid != 0) {
                           ...
                           // String InvoiceNo = pstBillMain.updateInvoiceNo(oidBillMain, invoiceCounter, invoiceNo);
								...
								if (billMain.getDocType() == 0 && billMain.getTransctionType() == 0
										&& billMain.getTransactionStatus() == 1 && billMain.getStatusInv() == 1){
									
									CashCashier entCashCashier = new CashCashier();
									try {
										entCashCashier = PstCashCashier.fetchExc(creditPaymentMain.getCashCashierId());
									} catch (Exception e) {
									}
									...
									String cashierNumberFormat = "SO"+entCashMaster.getCashierNumber()+"."+Formater.formatDate(new Date(),"yyyyMMdd");
									Vector listLastInvcNumber = PstBillMain.list(0, 1, ""+PstBillMain.fieldNames[PstBillMain.FLD_INVOICE_NO]+" LIKE '%"+cashierNumberFormat+"%' "
										+ "AND "+PstBillMain.fieldNames[PstBillMain.FLD_BILL_DATE]+" LIKE '"+Formater.formatDate(new Date(), "yyyy-MM-dd")+"%'",
										PstBillMain.fieldNames[PstBillMain.FLD_INVOICE_NO]+" DESC");
									String invoiceNumb = "";
									if(listLastInvcNumber.size() > 0){
										...
										billMain.setInvoiceCounter(newNumber);
							...
							if (billMain.getCustomerId()==0 && creditType.equals("1")){
