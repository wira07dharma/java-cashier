...
//                    billMain.setInvoiceNo(""+value);
     ...
     public int actionUpdateItem(int cmd,long oidBillDetail, int status) {
         return actionUpdateItem(cmd,oidBillDetail,status,"",0);
     }
     
     public int actionUpdateItem(int cmd,long oidBillDetail, int status, String note) {
         return actionUpdateItem(cmd,oidBillDetail,status,note,0);
     }
               ...
               case Command.APPROVE:
                    try {
                            ...
                            if(billDetail.getQty()==qytErr){
                                ...
                                double qtyIssue = billDetail.getQty() - qytErr;
                                if(qtyIssue<0){
                                    billDetail.setItemPrice(0);
                                    billDetail.setTotalPrice(0);
                                    billDetail.setNote(note);
                                    ...
                                    billDetail.setQty(qtyIssue);
                                    billDetail.setTotalPrice(billDetail.getItemPrice()*qtyIssue);
                                    billDetail.setNote(note);
                                    PstBillDetail.updateExc(billDetail);
                                    ...
                                    try{
                                        ...
                                        billDetail.setTotalPrice(qtyIssue*billDetail.getItemPrice());
                                        PstBillDetail.insertExc(billDetail);
                                    }catch(Exception ex){
                                    }
                                }
