...
public class  CtrlBillDetail  extends Control implements I_Language   {
    ...
    public int actionObject(int cmd, Billdetail billDetail,long oidBillMain) {
        msgString = "";
        int excCode = I_DBExceptionInfo.NO_EXCEPTION;
        int rsCode = RSLT_OK;
        switch (cmd) {
    ...
    public int action2(int cmd, long getBillDetail, long oidBillMain,long oidParent) {
        msgString = "";
        int excCode = I_DBExceptionInfo.NO_EXCEPTION;
        int rsCode = RSLT_OK;
        switch (cmd) {
            ...
            case Command.SAVE:
                ...
                frmBillDetail.requestEntityObject(billdetail);
                billdetail.setBillMainId(oidBillMain);
                billdetail.setParentId(oidParent);
                if (frmBillDetail.errorSize() > 0) {
                    msgString = FRMMessage.getMsg(FRMMessage.MSG_INCOMPLATE);
                    return RSLT_FORM_INCOMPLETE;
                }
