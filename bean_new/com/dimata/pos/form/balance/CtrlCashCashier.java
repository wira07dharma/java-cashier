                ...
                if (cashCashier.getOID() == 0) {
                    try {
                        ...
                        if(oid!=0){
                            ...
                            //CEK JUMLAH CURRENCY YANG DIGUNAKAN
                            int countCurrency=0;
                            countCurrency = FRMQueryString.requestInt(request, "FRM_FIELD_CURRENCY_COUNT");
                            for (int i = 1; i<=countCurrency;i++){
                                
                                //GET CURRENCY TYPE AND BALANCE EACH TYPE
                                long currencyID = FRMQueryString.requestLong(request, ""+FrmCashCashier.fieldNames[FrmCashCashier.FRM_FIELD_CURRENCY_ID]+"_"+i+"");
                                double balanceValue = FRMQueryString.requestDouble(request, ""+FrmCashCashier.fieldNames[FrmCashCashier.FRM_FIELD_SUBTOTAL1]+"_"+i+"");
                                
                                Balance balance = new Balance();
                                balance.setCashCashierId(cashCashier.getOID());
                                balance.setBalanceValue(balanceValue);
                                balance.setCurrencyOid(currencyID);
                                balance.setBalanceDate(new Date());
                                balance.setBalanceType(cashCashier.getType());
