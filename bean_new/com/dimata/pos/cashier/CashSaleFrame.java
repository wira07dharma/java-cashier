...
//GEN-FIRST:event_itemCodeTextFieldKeyPressed
    ...
    private AppUser userSalesPerson = new AppUser();
...
//        Vector rs = CashSaleController.getSales(0, 0, salesNameTextField.getText(), "");
          ...
          Vector rs = CashSaleController.getUserSales(0, 0);
        if (rs.size() > 0) {
            if (rs.size() == 1) {
                sales = (AppUser) rs.get(0);
            } else {
                CashSaleController.setUserSalesChoosen(null);
//                CashSaleController.setSalesChoosen(null);
                CashSaleController.showSalesSearch(this, salesNameTextField.getText(), "");
                sales = CashSaleController.getUserSalesChoosen();
//                sales = CashSaleController.getSalesChoosen();

            }
            if (sales != null && sales.getFullName() != null && sales.getFullName().length() > 0) {
                this.setUserSalesPerson(sales);
//                this.setSalesPerson(sales);
                this.getSaleModel().setUserSalesPerson(sales);
//                this.getSaleModel().setSalesPerson(sales);
                 this.getSaleModel().getMainSale().setAppUserSalesId(sales.getOID());
//                 this.getSaleModel().getMainSale().setSalesCode(sales.getCode());
                this.getBillMain().setAppUserSalesId(sales.getOID());
//                this.getBillMain().setSalesCode(sales.getCode());
                salesNameTextField.setText(sales.getFullName());
...
//GEN-FIRST:event_memberNameTextFieldActionPerformed
    ...
    public void cmdSelectSales() {
        if (CashierMainApp.isEnableSaleEntry()) {
            ...
            this.getSaleModel().getMainSale().setAppUserSalesId(0);
     ...
     * @return Value of property salesPerson.
     */
    public AppUser getUserSalesPerson() {
     ...
     * @param salesPerson New value of property salesPerson.
     */
    public void setUserSalesPerson(AppUser salesPerson) {
        this.userSalesPerson = salesPerson;
    }
