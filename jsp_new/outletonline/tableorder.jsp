...
<script language="JavaScript">
    ...
    function cmdChangeLocation(idRoom,capacity){
       var capacity = document.frmsrcsalesorder.capacity.value;
        $("#posts").html("");
        $.ajax({
            url : "<%=approot%>/servlet/com.dimata.cashierweb.ajax.CreateTableFromRoom?roomId="+idRoom+"&capacity="+capacity+"&locationId=<%=locationSales%>",
            type : "POST",
            async : false,
            cache: false,
...
<script language="JavaScript">
    $("#posts").html("");
        $.ajax({
            url : "<%=approot%>/servlet/com.dimata.cashierweb.ajax.CreateTableFromRoom?roomId=<%=selectFirstRoom%>&capacity=0&locationId=<%=locationSales%>",
            type : "POST",
            async : false,
            cache: false,
            success : function(data) {
