<%-- 
    Document   : kitchen_list_order_beverage
    Created on : Sep 29, 2015, 3:51:49 PM
    Author     : dimata005
--%>
...
<%
    ...
    int print = FRMQueryString.requestInt(request, "print");
    ...
    String where ="";
    if (1==1){
        where = " cbm.TRANSACTION_STATUS=1 AND cbm.TRANSACTION_TYPE=0 AND cbm.DOC_TYPE=0 AND bd.STATUS=0 and bd.STATUS_PRINT=1 and pm.CATEGORY_ID='504404582510052277' ";
    }else{
        where = " lc.LOCATION_ID = 0"+
                " AND cbm.TRANSACTION_STATUS=1 AND cbm.TRANSACTION_TYPE=0 AND cbm.DOC_TYPE=0 AND bd.STATUS=0 and bd.STATUS_PRINT=1 and pm.CATEGORY_ID='504404582510052277' ";
    }
    ...
    String whereRunning ="";
    if (1==1){
        whereRunning = " cbm.TRANSACTION_STATUS=1 AND cbm.TRANSACTION_TYPE=0 AND cbm.DOC_TYPE=0 AND bd.STATUS=1 AND bd.STATUS_PRINT=1 and pm.CATEGORY_ID='504404582510052277'";
    }else{
        whereRunning = " lc.LOCATION_ID = 0"+
                " AND cbm.TRANSACTION_STATUS=1 AND cbm.TRANSACTION_TYPE=0 AND cbm.DOC_TYPE=0 AND bd.STATUS=1 AND bd.STATUS_PRINT=1 and pm.CATEGORY_ID='504404582510052277'";
    }
    
    Vector listTransactionRunningOrder = PstBillMain.listItemOpenBill(0, 0,whereRunning, "");
...
<a href="kitchen_list_order_beverage.jsp?print=0">Print OFF</a> |  <a href="kitchen_list_order_beverage.jsp?print=1">Print ON</a>   
        ...
        <table role="grid" class="tablesorter-dropbox tablesorter hasFilters">
            ...
            <tbody aria-relevant="all" aria-live="polite">
                ...
                for(int i = 0; i < listTransaction.size(); i++) {
                    ...
                    <td align="left">
                        <a href="kitchen_list_order_beverage.jsp?FRM_FIELD_CASH_BILL_DETAIL_ID=<%=billDetail.getOID()%>&STATUS=2&UPDATE=1" style="color:#F90;"><em>CHECK OUT</em>
                        </a>
                    </td>
                </tr>
                <%
    ...
    <input data-lastsearchtime="1416898435043" class="form-control search selectable" placeholder="Search keyword.." data-column="all" type="search">
        <table role="grid" class="tablesorter-dropbox tablesorter hasFilters">
            ...
            <tbody aria-relevant="all" aria-live="polite">
                <%
                int startRunn = 0;
                for(int i = 0; i < listTransactionRunningOrder.size(); i++) {
                    Vector dataBill = (Vector) listTransactionRunningOrder.get(i);
                    Billdetail billDetail = (Billdetail)dataBill.get(0);
                    TableRoom table =(TableRoom)dataBill.get(1);
                    String statusItem="";
                    if(billDetail.getStatus()==1){
                        statusItem="Running Order";
                    }else if(billDetail.getStatus()==2){
                        statusItem="Check Out Kitchen";
                    }else{
                        statusItem="Reserved";
                    }
                    startRunn = startRunn + 1;
                    %>
                    <tr style="" class="tablesorter-hasChildRow odd">
                    <td style="text-align:center;"><a href="#" class="toggle"><%=startRunn%></a></td>
                    <td align="left"><a href="#" class="toggle"><%=billDetail.getItemName()%></a></td>
                    <td align="left"><a href="#" class="toggle"><%=billDetail.getQty()%></a></td>
                    <td align="left"><a href="#" class="toggle"><%=table.getTableNumber()%></a></td>
                    <td align="left"><a href="#" class="toggle"><%=billDetail.getNote()%></a></td>
...
<%if(print==1){%>            
<script language="JavaScript">
    window.open("<%=approot%>/servlet/com.dimata.posbo.printing.kitchenorder.PrintItemOrder?typePrint=1","corectionwh","scrollbars=yes,height=600,width=800,status=no,toolbar=no,menubar=yes,location=no");
</script>
<%}%>
</body></html>
